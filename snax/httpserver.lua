...
local function launch_slave(handler, protocol)
...
local function launch_master(conf)
	...
	skynet.error(string.format("http server listen at : %s %d", host, port))
...
local function http(conf)
    local name = "."..(conf.name or "http")
	skynet.start(function()
		local loginmaster = skynet.localname(name)
        if loginmaster then
            local handler = assert(conf.handler)
			launch_master = nil
			launch_slave(handler, conf.protocol or "http")
		else
			launch_slave = nil
